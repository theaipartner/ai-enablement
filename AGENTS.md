...
Primary context for anyone (human or AI agent — Codex, Claude Code, etc.) working on this repo. Read it
before making changes. `AGENTS.md` just imports this file; edit this one.
...
| Dev environment | macOS | Setup: `docs/runbooks/setup_mac.md`. Python in `.venv/`, Node via Homebrew. |
| Secrets | `.env.local` (local) + Vercel env vars (prod) | Get a local copy with `vercel env pull .env.local` (Sensitive vars come down blank — fill those from Bitwarden). `.env.example` is the template; the full account/credential inventory is in `docs/runbooks/credentials-and-accounts.md`. `SUPABASE_DB_PASSWORD` in `.env.local` lets ops scripts connect via psycopg2 (migrations, seeds, diagnostics). |
...
```bash
pytest tests/ -q        # Python suite — must pass before any commit
ruff check . && black --check .
npm run lint            # dashboard lint
npm run build           # dashboard type-check + build
npm run dev             # dashboard at http://localhost:3000 (set NEXT_PUBLIC_DISABLE_AUTH=true locally)
```

Pushing to `main` deploys to production. Ask the user before pushing.
