...
- **Vercel build-cache contamination — manual no-cache redeploy resolves 250MB function-bundle errors.** When a git-push auto-deploy errors with `"A Serverless Function has exceeded the unzipped maximum size of 250 MB"`, the failure is caused by Vercel restoring a contaminated build cache from a prior deployment (likely seeded by a `vercel deploy` from local that uploaded the working dir's untracked Python pkg dirs into each function bundle). Recovery: dashboard "Redeploy" with **Use existing Build Cache** unchecked — the no-cache build produces clean bundles and uploads a replacement cache. The next git-push deploy then restores the now-clean cache and succeeds. Root-caused 2026-05-08 via Phase 3b discovery (build-log diff of commit d14770e's failed git-push vs successful no-cache redeploy); prevention work (55 junk pkg dirs deleted, `data/` added to .vercelignore) landed in the same session. **Drake currently handles deploys manually as a standing pattern** — provisional. Once a clean post-fix push validates, deploys move into Director's lane via the git-push side effect (push is a Director gate, so the auto-deploy that follows is too — see § Director / Builder System § Gate trajectory). See `docs/known-issues.md` for the full diagnostic signature + recovery steps.
...
- **Deploys — operationally Drake-handled TODAY, but NOT a structural gate (workaround for a bug, not an intentional gate decision).** Drake's pattern of clicking dashboard "Redeploy" after auto-deploy errors was an operational response to the Vercel build-cache contamination 250 MB pattern (root-caused 2026-05-08 via Phase 3b discovery, see § Operational patterns above and `docs/known-issues.md`), NOT an intentional gate. **Structurally, deploys were never Drake-gated:** git-push triggers Vercel auto-deploy via the GitHub integration, and push is already a Director gate (push is reversible — see § Drake's gates). So when push works → deploy works → deploys live in Director's lane as a side effect of push, not because Drake explicitly demoted the gate. **Pending validation:** the next natural git-push after Phase 3b's fix session needs to deploy cleanly without manual redeploy (validation that the contaminated cache replacement landed today is durable). **After validation:** the "Drake handles deploys manually" line in § Operational patterns relaxes, and this trajectory bullet retires entirely — deploys aren't moving into a different gate category, they're being recognized as never having been structurally gated. **If validation fails:** the bug is deeper than cache contamination; run the failure-case investigation per `docs/known-issues.md` § Vercel build cache entry (likely a local `vercel build --debug` to inspect actual function-bundle composition).
