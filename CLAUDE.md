...
- **(b) Living-doc updates** — `docs/state.md` (per-batch shipped detail) gets updated in the same Builder commit-sequence as the batch that shipped it. CLAUDE.md § Current Focus + § Next Session Priorities get updated if the in-flight arc shifted. See § Update Policy for This File.
