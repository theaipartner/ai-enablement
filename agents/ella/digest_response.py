...
counts + cost. The caller (`passive_dispatch` / `agent`) detects the
`[FALLBACK_TO_SONNET]` token (anywhere in the output) and routes to
Sonnet when present.

The system prompt is a trimmed version of
`agents/ella/prompts.py:_BASE_PROMPT` — same Ella voice, same Slack
mrkdwn formatting discipline, tightened for short answers, with an
explicit instruction to fall back to Sonnet on any uncertainty. The
decision Haiku already validated this message as a clean answerable
program question before routing here, so the decline / escalate
sections of the full prompt are intentionally absent — fallback is
the only escape hatch the response Haiku needs.
"""
...
# Literal token the response Haiku emits when it can't answer well. The
# caller detects this anywhere in the output, discards the response,
# and routes the question to Sonnet via the pending_ella_responses
# path. The client never sees the token.
_FALLBACK_TOKEN = "[FALLBACK_TO_SONNET]"
...
# ESCALATE replaced with WHAT YOU DO IF YOU CAN'T ANSWER. FIRM AFTER
...
def generate_response(
    *,
    ...
    fallback = _FALLBACK_TOKEN in text
    return DigestResponseResult(
        response_text=text,
