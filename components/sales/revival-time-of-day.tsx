import type { RevivalHourBucket } from '@/lib/db/funnel-revival'

// Revival time-of-day — replies vs dials vs connects in 2-hour ET buckets. Pure
// SVG grouped bars so it renders server-side. Wall-clock (no business-hours
// adjustment): the gap between when leads reply and when we dial/connect is the
// whole point. See lib/db/funnel-revival.ts getRevivalTimeOfDay.

const C_REPLY = '#7ea8dd' // blue — lead replied
const C_DIAL = '#d08770' // coral — setter dialed
const C_CONNECT = '#a3be8c' // green — connected

const SERIES: { key: keyof Pick<RevivalHourBucket, 'replies' | 'dials' | 'connects'>; color: string; label: string }[] = [
  { key: 'replies', color: C_REPLY, label: 'Replies' },
  { key: 'dials', color: C_DIAL, label: 'Dials' },
  { key: 'connects', color: C_CONNECT, label: 'Connects' },
]

export function RevivalTimeOfDaySection({ buckets }: { buckets: RevivalHourBucket[] }) {
  ...
  const CHART_W = 720
  const CHART_H = 250
  const max = Math.max(1, ...buckets.flatMap((b) => [b.replies, b.dials, b.connects]))
  ...
  return (
    <div style={{ marginTop: 14, border: '1px solid var(--color-geg-border)', borderRadius: 8, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'var(--color-geg-bg-elev)', borderBottom: '1px solid var(--color-geg-border)' }}>
        <span style={{ width: 8, height: 8, borderRadius: '50%', background: C_REPLY }} />
        ...
        <span className="geg-mono" style={{ fontSize: 9, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--color-geg-text-faint)' }}>
          · replies vs dials vs connects · ET
        </span>
      </div>
