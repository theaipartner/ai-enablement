...
| `latest_nps` | `clients.nps_standing` (Airtable mirror via NPS-is-gospel auto-derive, migration 0027) | promoter→100, neutral→50, at_risk→0 | 0.20 | "no NPS standing" → neutral 50 with distinct "no record" note (vs. neutral segment's "(passive)" note — same contribution, distinct semantics) |

Weights sum to 1.0. Heavy-but-balanced — the AI call signal dominates at half the weight; the deterministic floor handles cadence + NPS while overdue trims to 0.10 since the open-but-not-yet-due count was double-counting and is retired in V2. **Slack engagement** is still absent (the `slack_messages` cloud table is empty per `docs/archive/historical/future-ideas.md`); land it as a fifth signal once cloud Slack ingestion ships and re-balance.
...
`SweepResult` carries `duration_ms` + `avg_per_client_ms` populated at sweep completion. The sweep also emits an INFO log line with these values so cron logs surface the trend without requiring `agent_runs` queries. Feeds the cron-ceiling watchpoint logged in `docs/archive/historical/known-issues.md` — if a daily sweep ever exceeds 240s (80% of 300s), the freshness filter isn't doing enough and we re-evaluate (parallelize the per-client loop OR move the AI signal to a separate weekly job OR upgrade the plan).
...
| Journey stage | `clients.journey_stage` | Six values via 0028 CHECK: business_setup / business_setup_activation_done / prospecting / first_closing_call_taken / first_closed_deal / ten_k_month. Display labels via `JOURNEY_STAGE_LABEL` lookup. |
...
Vertical layout, 7 collapsible sections via native `<details>`/`<summary>`. **Default-open state** (per `defaultOpen` prop on `components/client-detail/section.tsx`): five sections default expanded (Identity & Contact / Lifecycle & Standing / Activity & Action Items / Adoption & Programs / Notes), two sections default collapsed (Financials / Profile & Background) — the latter pair are under-used today and clutter the page when expanded by default; CSMs click to expand on the rare occasion they need them. Always-collapse-on-page-load (no localStorage / per-user state). The structure changed in M4 Chunk B (post-migration 0017) — what was a 6-implicit-section layout reorganized into 7 explicit sections that surface the new schema (14 columns + nps_submissions.recorded_by + 4 new tables). M4 Chunk B2 wires inline-save on every editable field: click swaps the read-only display into an input/select/textarea, blur or Enter saves, Escape cancels. Status / journey_stage / csm_standing route through history-writing RPCs (migration 0018) so every edit leaves an audit row in the corresponding `*_history` table. metadata.profile.* fields go through a read-modify-write on `clients.metadata`. The needs_review tag triggers a Merge button at the top of the page (orthogonal to the sections, preserved from M3.2).
...
**Section 2 — Lifecycle & Standing:** CSM-judgment fields plus system-derived signals. Editable-in-B2: journey_stage (enum dropdown post-2026-05-08 / migration 0028: business_setup / business_setup_activation_done / prospecting / first_closing_call_taken / first_closed_deal / ten_k_month — replaced the V1 free-text shape; pre-apply data was 100% NULL so the CHECK landed clean), csm_standing (enum: happy/content/at_risk/problem — new), archetype (new — free-text V1, enum once Drake/Nabeel finalize). System-derived: NPS standing pill (renders `clients.nps_standing` via the `NpsStandingPill` component — replaced the prior raw-NPS-score field that read `nps_submissions.score` since that table stayed empty in production); Health score from latest `client_health_scores` row (preserved indicator with tier pill, "why this score" expand of the factors jsonb); and Concerns as a collapsible sub-section under Health score that distinguishes three empty states: "Gregory has not yet evaluated this client" when no health row exists, "No concerns currently surfaced" when a health row exists but `factors.concerns[]` is empty, and the existing list rendering (text + severity pill + linked source calls) when concerns are present.
...
Removed via spec `remove-ella-runs-page`. Shipped 2026-05-11 as a read-only audit surface at `/ella/runs` (list) + `/ella/runs/[id]` (detail) — the visual redesign followed on 2026-05-14, including the editorial header / 420-1fr grid / `geg-slack-msg` rendering described in `docs/archive/historical/state.md`. Removed once the 2026-05-23 @-mention-split made the passive path observation-only (digest + unanswered-flagger only); no per-run review surface had a purpose. Route directory `app/(authenticated)/ella/`, query layer `lib/db/ella-runs.ts`, and three Playwright verify scripts (`scripts/verify-ella-*.ts`) all deleted in the same spec. Any future audit is via direct SQL on `agent_runs` (filter `agent_name='ella'`).
...
3. **`update_client_from_nps_segment` RPC** (migration 0021, replaced in 0027) — does the work in one transaction. Always writes `clients.nps_standing`. **Always** auto-derives `clients.csm_standing` from the segment via `update_client_csm_standing_with_history` (NPS-is-gospel — flipped from override-sticky in 0027).

**NPS-is-gospel semantics (post-2026-05-08, migration 0027):**

Every NPS segment write propagates to `csm_standing` unconditionally. There's no override-sticky check, no Gregory-Bot-history gate, no manual-judgment-wins branch — the segment is the source of truth. CSMs can still set `csm_standing='problem'` manually because no segment maps to 'problem', but every other manual override gets overwritten on the next NPS submission for that client.

Idempotency stays in place via the underlying `update_client_csm_standing_with_history` RPC's `IS NOT DISTINCT FROM` check — when the derived value already matches `clients.csm_standing`, no `client_standing_history` row is written. So the dashboard isn't spammed with redundant audit rows on every NPS re-fire. Real-write attribution still flows through Gregory Bot UUID (`cfcea32a-062d-4269-ae0f-959adac8f597`), now with note string `'auto-derived from NPS segment <segment> (NPS-is-gospel)'` so post-flip writes self-identify in audit queries.

**Why the flip.** Override-sticky shipped at M5.4 (2026-05-03) per Scott-confirmed behavior B — "manual CSM judgment wins." Reversed 2026-05-08 per Drake's NPS-is-gospel call: NPS Survey segments are the operational signal CSMs should be aligning to, not the other way around. The pre-apply dry-run identified 16 rows that needed realignment (14 master-sheet seeds + 2 stale Gregory Bot auto-derives, zero current CSM manual overrides) — the backfill at the bottom of 0027 ran those flips in one shot. The `Gregory Bot` `team_members` row (added in 0021, role `system_bot`) is still useful as auto-write attribution but no longer gates auto-derive.
...
`auto_derive_applied` is **always `true` on the 200 success path** post-0027 (NPS-is-gospel). The RPC always auto-derives; the receiver returns the constant. Field preserved in the response shape so Make.com consumers aren't broken by a deletion. Source of truth for whether a history row actually landed remains `client_standing_history.changed_by` — Gregory Bot UUID on the most recent row indicates the auto-derive wrote it (vs. the underlying RPC's idempotency taking a no-op when value unchanged).
...
**Local test harness:** `scripts/test_airtable_nps_webhook_locally.py` spins up the real `handler` class via `http.server.HTTPServer` in a background thread (same pattern Vercel uses), fires 10 paths (2 happy + 2 NPS-is-gospel + 6 negative), verifies HTTP responses + cloud DB state via direct psycopg2, hard-deletes the seeded test fixture in try/finally. Self-seeded fixture pattern (post-2026-05-08) — RUN_TOKEN-suffixed unique email per invocation, mirrors `scripts/test_airtable_onboarding_webhook_locally.py`. Replaced the prior static-Branden fixture pattern after Branden was archived in the M5 misclassification cleanup (2026-05-05). Run via `.venv/bin/python scripts/test_airtable_nps_webhook_locally.py`. Sets a test secret if `AIRTABLE_NPS_WEBHOOK_SECRET` is unset.
...
Shipped: two CS-visibility features bundled in one session as the first slice of Batch A — CSM accountability visibility (per `docs/archive/historical/future-ideas.md` § Batch A). The two features share infrastructure (a new `shared/slack_post.py` helper extracted from Ella's two-token post path) and audit pattern (`webhook_deliveries` rows with new source labels) but live in separate trigger surfaces.
...
**Architectural finding logged to `docs/archive/historical/known-issues.md`** so future sessions know why all crons share a single secret: independent per-cron rotation isn't supported by Vercel. If a future use case ever requires true independence (e.g., a third-party caller who shouldn't be able to trigger ALL crons by knowing one secret), it requires a separate auth surface — not solvable via env var naming.

## Current state snapshot (extracted from CLAUDE.md, 2026-05-11)

This snapshot lifts the orientation paragraph that previously lived in CLAUDE.md § Gregory (active focus). Read it for a single-page "what is Gregory today" view; the rest of this file (Brain V2, Pages, Build log) carries the deeper detail. Future updates to this snapshot ride in the same Builder commit-sequence that ships the next Gregory batch.
...
**Hosting.** Single Vercel project at `https://ai-enablement-sigma.vercel.app`. Mixed-framework: Next.js 14 dashboard at repo root + 9 Python serverless functions. `vercel.json` declares `"framework": "nextjs"` (required to suppress framework auto-detection when `functions` is also explicit) plus per-file Python runtimes. Each Python function carries `"excludeFiles": "{.next,node_modules}/**"` to drop Vercel-build-phase Next.js artifacts from the function bundle (.vercelignore can't reach those — they're created after the ignore is applied). See `docs/runbooks/vercel_python_bundle_size.md` when adding a new Python function or when the 250 MB error message recurs. Vercel Cron schedules: daily 08:00 UTC → fathom_backfill; daily 09:00 UTC → gregory_brain_cron (switched from weekly Mondays on 2026-05-08 paired with the AI-signal freshness filter); daily 12:00 UTC → accountability_notification_cron; per-minute → passive_ella_cron (Ella V2 Batch 2.3 passive-monitor queue drainer).
