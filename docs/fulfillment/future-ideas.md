...
For Ella's deferred work see `docs/agents/ella/future-ideas.md`. For Gregory's known bugs and ops gaps see `docs/fulfillment/known-issues.md`. For architectural decisions, see `docs/decisions/` (when populated).
...
Delivered. V2 rubric: `ai_call_signal` 0.50 + `call_cadence` 0.20 + `overdue_action_items` 0.10 + `latest_nps` 0.20. The AI signal is the dominant qualitative contributor; `open_action_items` retired (was double-counting with overdue + the AI signal's read of action items). Calibration followup logged in `docs/fulfillment/known-issues.md`.
...
The specific FP entries (hiring-interview, representative-of-existing-client, iMIP) live in `docs/fulfillment/known-issues.md` with reframe notes — they'll get pulled into Batch D work if the titling rollout doesn't suppress them.
