...
Real bugs and ops gaps for Gregory, with concrete next actions. Distinct from `docs/fulfillment/future-ideas.md` (Gregory V2 batches A–E — deferred features waiting on a trigger) and `docs/decisions/` (architectural decisions, when populated). Ella's bugs and ops gaps live separately in `docs/agents/ella/followups.md` (kept under that name for now; renamed when an Ella-focused sweep happens).
...
**This is a one-time verification gate, not a recurring routine.** First action of the next session, before any planned work. Remove this entry from `docs/fulfillment/known-issues.md` AND the matching pointer from `CLAUDE.md § Next Session Priorities` once the verification has run, regardless of outcome.
...
## ~~Passive dispatch has no idempotency check against duplicate Slack message delivery~~ — RESOLVED 2026-05-21

**Resolution note (2026-05-21):** The 2026-05-20 ship (`ella-realtime-ingest-idempotency`) shipped the correct dedup architecture but with the wrong key. It built the webhook_id from `event.get("ts")` (outer event ts), which differs between an original `message` event and its follow-on `message_changed` edit event — so the PK collision the gate relied on never fired in production for edits. The 2026-05-21 diagnostic (`docs/reports/ella-duplicate-webhook-delivery-diagnostic.md`) documented 11 duplicate dispatches / 8 channels / 36 hours with zero forensic-dup rows ever written. The actual fix shipped via `ella-realtime-ingest-dedup-message-changed` (2026-05-21) — step 0 now runs POST-parse and keys on `record.slack_ts` (inner/canonical ts, stable across edits). Production resume on the 136 paused channels is gated on Drake's smoke validation of the new gate.

Original architecture description (left in place for context):
...
- **Next action:** Operational, not code. Ensure Ella's bot is invited to every client channel the flagger surfaces from. The existing `scripts/invite_ella_and_bot_to_client_channels.py` (dry-run + apply) is the relevant rollout tool; full-fleet rollout was pending Drake-led ops work as of the 2026-05-10 Batch 1 wrap (`docs/fulfillment/state.md`). No spec needed — this entry exists so the next person doesn't waste time looking for a code fix.
- **Logged:** 2026-05-21 (surfaced during the post-resume EOD review of the unanswered-flagger ship).

## ~~Cost hub current-month total used a different code path from history total — co-edit divergence~~ — RESOLVED 2026-05-23 (cost-hub-current-month-total-fix)

Resolved structurally via `docs/specs/cost-hub-current-month-total-fix.md`. The history view's `getMonthTotal` correctly used `fetchSubscriptionsForHistory` (archive-inclusive) + the `subscriptionActiveInMonth` overlap predicate, but the current-month total in `page.tsx` used `getMonthlySubscriptions` (archive-excluded) AND hardcoded `archived_at: null` when passing rows through the same predicate. Net effect: a sub archived mid-month (e.g. ElevenLabs on 2026-05-23) wrongly vanished from May's running total even though Drake paid for it that month. The fix added `getSubscriptionsActiveInCurrentMonth` (calls the renamed `fetchAllSubscriptionsWithArchive` + the same predicate) and split `page.tsx` into two derived lists: `editableSubscriptions` (archive-excluded) for the editable table, `subsActiveInMonth` (archive-inclusive) for the total. Symmetric `getCurrentMonthExtrasForTotal` for extras. Report: `docs/reports/cost-hub-current-month-total-fix.md`. Runbook: `docs/runbooks/cost_hub.md` § "Editable table vs running total." Same shape of co-edit divergence as the Ella prompt drift (`docs/agents/ella/followups.md` § "Co-edit risk on Ella's split prompts") — flagging here too in case a future code-organization review wants to design a structural guard for one-source-of-truth-two-views patterns more broadly.

## ~~Cost hub Playwright verifier — add archive-still-counts assertion for extras~~ — RESOLVED 2026-05-24 (verifier rewrite)

Resolved via the `cost-hub-total-cancel-remove-and-add` verifier rewrite. The new verifier asserts the sum-to-total invariants for both surfaces: step 4d (cancel-invariant — Cancel a test sub, assert it stays visible with "Cancelled" badge AND total is unchanged), step 5 (remove-invariant — × Remove a test sub, assert it's gone AND total dropped by exactly the test cost), step 6 (extra Remove — symmetric for extras). The earlier per-extras gap is closed by treating ADD-DELTA + REMOVE-DELTA as the universal invariants regardless of row type. The rewrite also moved cleanup from soft-archive to hard-DELETE in try/finally so verifier runs never pollute production totals again (see next entry).

## ~~Cost hub running total polluted by Playwright verifier soft-archived rows~~ — RESOLVED 2026-05-24 (rewrite + one-time data cleanup)

The 2026-05-23 current-month-total fix (`cost-hub-current-month-total-fix`) correctly started counting archived-this-month rows in the running total. As a side effect, **10 leftover `__verify_*` test rows from the 2026-05-15 Playwright verifier run** (6 subs at $89.36 + 4 extras at $31.56) — which had been soft-archived after testing and silently lurking — suddenly counted toward May 2026's total, inflating it from a real ~$162 to a polluted $308.93. Drake's "doubled total" report turned out to be the math being CORRECT given the pollution. Two-part resolution:

1. **One-time data hygiene:** 10 `__verify_*` rows hard-DELETEd via direct SQL (within `cost-hub-total-cancel-remove-and-add` Part 1 scope — clearly-prefixed test artifacts by the verifier's own labelling convention, never Drake's real data). Post-cleanup total = $188.01.
2. **Structural fix:** `scripts/verify-cost-hub-preview.ts` rewritten to hard-DELETE its `__verify_*` test rows in a try/finally cleanup step (via direct Supabase admin SDK call so it works even if the test fails mid-run). The prior soft-archive cleanup was safe pre-Q1-fix because archived rows didn't count; post-Q1-fix soft-archive cleanup is silent pollution. The new cleanup pattern: tests can soft-archive or hard-delete via the UI flows being tested, but the AT-RUN-END cleanup is always hard-DELETE of every `__verify_*` row.

Logged here because it's a meaningful operational learning: **test data semantics changed when the underlying total math changed.** The verifier's cleanup that was "fine" before became "actively polluting" after. Worth noting whenever a query semantic widens — does any test infrastructure rely on the prior narrower semantic? See `docs/reports/cost-hub-total-cancel-remove-and-add.md` for the full diagnosis.
...
- **Next action:** when V2 generation lands (or before, if anyone else touches the persistence layer): add a migration that extends `match_document_chunks` to also exclude `document_type='call_review'` from global-mode results, and update `docs/fulfillment/metadata-conventions.md` §7 in the same commit.
- **Logged:** 2026-05-07.
