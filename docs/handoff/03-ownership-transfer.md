...
Session 3 is nearly complete. **Done: credentials, GitHub, Supabase, Vercel transfer + Git reconnect.** Remaining: confirm auto-deploy + post-transfer cleanup.

**DONE:**
- **Credentials** → company Bitwarden, one secure note holding the full prod env set. Local master file shredded. Recovery used: 38 pulled from Vercel + 13 from `.env.local` + Fathom×2 from Drake's personal Bitwarden + `SALES_FORM_NOTIFY_SLACK_CHANNEL` looked up; `ONCEHUB_WEBHOOK_SECRET` dropped (dead); **`CRON_SECRET` freshly generated** (`openssl rand -hex 32`) — stored in the Bitwarden note. **NOTE:** the env vars carried over with the Vercel transfer (see below), so prod still uses the **OLD** `CRON_SECRET`; Bitwarden's fresh value is currently unused. Reconcile the note to the old value (decision: keep old, no rotation).
...
- **Vercel** → project transferred to the company team **`success-projects-9dcde12c`** ("success' projects"). **URL unchanged (`ai-enablement-sigma.vercel.app`) → the 5 webhooks need no re-registration (Section B skipped).** Surprise: **env vars DID carry over** (all ~55 intact, original timestamps), so no re-add and no config-gap downtime. Git **reconnected** to `theaipartner/ai-enablement` → auto-deploy restored. Decisions: kept the OLD `CRON_SECRET` (no rotation); left the 6 dead vars in place (`ONCEHUB_WEBHOOK_SECRET`, `FATHOM_BACKFILL_AUTH_TOKEN`, `GREGORY_BRAIN_CRON_AUTH_TOKEN`, `ACCOUNTABILITY_NOTIFICATION_CRON_AUTH_TOKEN`, `ESCALATION_RECIPIENT_SLACK_USER_ID`, `ELLA_DAILY_DIGEST_CC_SLACK_USER_ID`) — harmless, 0 code refs.

**NEXT — confirm + clean up:**
1. **Verify auto-deploy** — push a trivial commit to `main` → confirm it deploys under the company team. *(in progress)*
2. **Verify crons + webhooks** — all 19 crons registered in the dashboard; a cron fires (`webhook_deliveries` audit rows); edit an Airtable record → row updates.
3. **Downgrade** Drake's personal Supabase + Vercel orgs (stop paying). Optionally remove Drake from the company Vercel team once env work is confirmed.

**AFTER Vercel:** Non-code IT item (already flagged in §C Google Cloud + Session 2 §A.1): **keep the `drake@theaipartner.io` work account ACTIVE through offboarding** — the calendar OAuth depends on it. Drake hands over his Google account login later.
