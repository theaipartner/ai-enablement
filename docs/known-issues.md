...
## Cost hub Playwright verifier — add archive-still-counts assertion for extras

- **What:** `scripts/verify-cost-hub-preview.ts` § step 5 was added 2026-05-23 to assert "sub archived mid-current-month is gone from the editable table AND still in the running total" end-to-end. The earlier verifier (2026-05-15) added + archived test rows but never asserted the dollar amount in the total after archive — that gap let the original bug ship unnoticed. The new step closes the gap for the SUBSCRIPTION side. **Extras have NO equivalent end-to-end assertion** of the same invariant.
- **Why it matters:** the extras code path is structurally the same shape as subs (the fix added `getCurrentMonthExtrasForTotal` parallel to `getSubscriptionsActiveInCurrentMonth`). A symmetric verifier assertion would catch any regression that breaks one but not the other.
- **Next action:** extend `verify-cost-hub-preview.ts` § step 6 with a "extra-archived-still-counts" sub-step, mirroring step 5: add an extra with a distinctive cost, capture the total, archive it, assert the total is unchanged + the editable table no longer shows it. Small spec; bundle with another cost-hub change if convenient.
- **Logged:** 2026-05-23 (during the current-month-total fix).
