...
## Vercel build cache can carry forward bloated function bundles, surfacing as 250 MB errors on git-push deploys

- **What:** Vercel's per-deployment build cache restores artifacts from the previous successful deployment by default. If a prior deploy attempt seeded the cache with bloated function bundles (e.g., a `vercel deploy` from local that uploaded the working dir's untracked Python pkg dirs into each `@vercel/python` function bundle), every subsequent **git-push** auto-deploy restores those bloated bundles and fails the 250 MB unzipped per-function size check during the "Deploying outputs" phase. The same commit redeployed via the dashboard with **Use existing Build Cache** unchecked builds fresh from the git tree only (which doesn't contain those untracked dirs), produces normal-sized bundles, and uploads a clean replacement cache.
- **Diagnostic signature** (from comparing failed git-push vs successful no-cache redeploy of commit `d14770e`, 2026-05-08):
  - Failed build: `Restored build cache from previous deployment (<cache-id>)` early in the log; build itself "Completes" successfully; failure surfaces during "Deploying outputs..." with the literal `'A Serverless Function has exceeded the unzipped maximum size of 250 MB'`.
  - Successful redeploy of same commit: `Skipping build cache, deployment was triggered without cache.` early in the log; full fresh `npm install`, fresh `@vercel/python@4.3.1` builder install; deploy completes; uploads a fresh `[~120 MB]` build cache that replaces the contaminated one.
- **Recovery — already used at least once in this repo:** From the Vercel dashboard, click "Redeploy" on a failed deployment and **uncheck "Use existing Build Cache"** in the dialog. The successful no-cache redeploy uploads a fresh cache that replaces the contaminated one, so subsequent git-push deploys should succeed automatically without intervention.
- **Prevention** (landed 2026-05-08 alongside this entry):
  - 55 untracked, non-functional Python pkg directory skeletons at the repo root (`anthropic/`, `openai/`, `pyiceberg/`, `cryptography/`, `pydantic/`, `hive_metastore/`, plus 49 more — none had `__init__.py`, all dated `Apr 28 15:54`, ~37 MB combined) deleted. They were almost certainly left over from an interrupted `pip install --target .` or wheel extraction; uploaded by `vercel deploy` from local but not by git push (untracked). Removing them prevents future local-deploy attempts from re-seeding the same contamination.
  - `data/` (~13 MB of ingestion run logs) added to `.vercelignore` so it can't be uploaded by `vercel deploy` from local even though it's gitignored from git push.
- **Why this surfaced 2026-05-07/08 specifically:** the contamination likely entered the cache pipeline around 2026-04-28 (the file-system date on the deleted dirs and on `.vercel/output/` from a local prebuild attempt). Function bundles may have been just under 250 MB for ~10 days as `shared/`, `agents/`, `api/` grew with brain-V2 / call-reviewer / accountability-cron work; some recent commit pushed one bundle past the threshold and the pattern started failing visibly. The exact triggering commit isn't pinned because the failed log Vercel served was truncated before the per-function size details — the diff between the failed and successful logs was diagnostic enough without it.
- **Earlier-symptom note (2026-04-29 M3.3 deploy):** the M3.3 push exhibited a different surface symptom — `status ● Error` with an empty Builds tree, no error message in the log, production alias unchanged. Resolved by a same-commit redeploy. May or may not share the cache-contamination root cause; insufficient evidence to confirm. If the same surface symptom recurs, suspect cache contamination first and try the no-cache redeploy before opening a Vercel support ticket.
- **Logged:** 2026-04-29 (M3.3 — original empty-builds-tree symptom); rewritten 2026-05-08 (Phase 3b discovery — root-caused via build-log diff of commit d14770e's failed git-push vs successful no-cache redeploy).
