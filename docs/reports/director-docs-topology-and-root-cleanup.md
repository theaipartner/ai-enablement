...
- **The spec was delivered via the exact race it documents.** `git log -- docs/specs/director-docs-topology-and-root-cleanup.md` returned only the report-landing commit. The spec file was on Drake's local disk via filesystem MCP but had never been pushed to `origin`. Drake's `/run` hint at the top of the session ("this file was written directly to the filesystem, no gitpull needed") was the live tell. Builder picked up the spec from local disk and committed it for the first time as part of the same commit that flipped Status to shipped. On-the-nose confirmation that the topology rule the spec introduces was necessary — the new rule's safety property is that "new spec files don't conflict with existing local working-tree state," and that's exactly what allowed `/run` to work here despite the missing remote-commit.
