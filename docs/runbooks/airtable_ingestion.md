...
- **`FULL_RESCAN_TABLES`** (pipeline.py, 2026-08-14): tables whose EDITS to old records matter ignore the created-time window and re-pull everything each tick. Currently the DC sale form (`tbljmzRoMoE5B26lt`, ~1.1k records ≈ 11 API pages) — its `Valid` QA verdict is edited on old records, and relying on the edit webhook proved fragile (it silently died ~Jul 25) — plus both EOD tables (`tblnGf0NoNCWVwOsz` Setter, `tbly2S13lmo82xy5e` Closer; added 2026-08-15 for the DC Ads rep EOD dropdown — reps edit reports after submitting, together ~150 records ≈ 2 API pages). Keep this set small-table-only.
