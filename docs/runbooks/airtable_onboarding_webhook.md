...
Country isn't validated against a vocab — Zain owns the contract. Today's expected values are `'USA'` / `'AUS'`, but anything else passes through as-is. (See followup in `docs/archive/historical/known-issues.md` about `/clients` filter drift if Zain ever sends `'United States'`.)
