...
- **PostgREST page cap at 1000 rows.** `db.table("x").select("id").execute()` silently returns at most 1000 rows. For accurate counts, use `db.table("x").select("id", count="exact", head=True).execute().count`. See `docs/known-issues.md` for the writeup.
