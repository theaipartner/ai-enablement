...
- **New-convention title cutoff (2026-05-18 00:00 EST):** calls with `started_at` at or after the cutoff only auto-classify as `call_category='client'` when the title matches one of the six canonical patterns generated by Zain's booking links: `Coaching Call with {Scott|Lou|Nico}` or `Sales Call with {Scott|Lou|Nico}` (case-insensitive prefix match). Post-cutoff calls with ad-hoc titles fall through to `internal` / `external` / `unclassified` based on participant shape — participant match no longer promotes to `client` post-cutoff. Pre-cutoff calls use the prior cascade unchanged; no retroactive reclassification. Manual override via `update_call_classification` RPC still works for edge cases. See `docs/runbooks/call_title_convention.md` and `docs/specs/classifier-enforce-new-title-convention.md`.
