...
`p_changed_by` and `p_recorded_by` on the M4-era functions are nullable. Auth context isn't wired through Server Actions in V1; every history row currently records `changed_by = null`. Followup logged in `docs/known-issues.md` to wire `auth.users → team_members` resolution.
