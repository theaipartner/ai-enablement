...
| `test_mode` | `boolean` | Default `false`. Per-channel test mode for passive monitoring. When `true`, the passive monitor's author-type gate accepts `team_member` messages in addition to `client` messages so a team member can smoke-test Ella as themselves. NEVER enable on a production client channel — test_mode runs are tagged in `agent_runs.trigger_metadata.test_mode_run=true` for audit-filter purposes. Added in migration 0031. |
...
- `api/passive_ella_cron.py` (re-checks the per-channel toggle before draining each pending row — it may be flipped off during the 1-minute queue wait)
...
Enable passive monitoring for a specific channel:
...
Enable test_mode for smoke testing (intended for the Ella smoke-test channel only):
