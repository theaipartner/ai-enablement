...
def render(runs: list[dict], escalations: dict, channels: dict, users: dict, response_texts: dict, thread_msgs: dict, flags: dict) -> str:
    ...
    w("")
    w(f"- **V1 pilot was strictly single-channel.** All {total} runs hit `C0AUWL20U8J` (#ella-test-drakeonly), not the 8 channels the spec's context paragraph implied. AGENTS.md § Ella confirms — V1 pilot is `#ella-test-drakeonly` awaiting Nabeel feedback before rolling out to the remaining 6 channels. So the channel-fanout dimension of speaker-mis-ID risk hasn't been pressure-tested yet; once Batch 2 + the channel rollout land, the same `slack_channels.client_id`-as-default-name path will affect every channel.")
    ...
    w(textwrap.dedent("""\
        - **The spec assumed runs across 8 V1 pilot channels.** Reality: all
          28 are in one channel. AGENTS.md § Ella is the authoritative source
