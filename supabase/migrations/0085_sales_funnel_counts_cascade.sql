-- 0085_sales_funnel_counts_cascade.sql
-- Adds Campaign + Ad-Set funnel filtering to sales_funnel_counts, for the
-- 3-level ad cascade (Campaign -> Ad Set -> Ad). Two new params + two filter
-- lines on the cyc CTE; close_leads is already joined there, so campaign_id /
-- adset_id come straight from it (no new columns, no tagger change). The ad
-- filter (lc.ad_id) is unchanged. inwin/tmem/anchors/dials all derive from the
-- filtered cyc, so dial attribution scopes correctly with no further edits.
...
-- Drop the prior 3-arg signature first: adding params changes the signature,
-- so create-or-replace would otherwise leave a stale overload (ambiguous calls).
drop function if exists sales_funnel_counts(timestamptz, timestamptz, text);
