-- 0116_outbound_by_rep_shape_fix.sql
-- Fix a return-shape regression introduced by 0115. When 0115 added the GHL call
-- arm to outbound_funnel_by_rep, it was rebuilt from the 0104 body (a BARE reps
-- array) and dropped the { reps, totals } wrapper that 0105 had added. The TS
-- consumer (getOutboundByRep in lib/db/funnel-revival.ts) reads d.reps / d.totals,
-- so a bare array made d.reps undefined → the per-rep table rendered EMPTY and the
-- header plan-mix chips lost their numbers. (The cohort funnel was unaffected —
-- that's outbound_funnel, a different function.)
--
-- This restores 0105's exact { reps, totals } shape (incl. the dc_forms-based
-- totals / plan mix) AND keeps 0115's GHL call arm (ghl_messages.user_id ->
-- team_members.ghl_user_id), so Close + GHL dials/connections merge per rep.
-- Closes/cash + totals are already source-agnostic (Airtable joined on the lead's
-- native id, which is the GHL contact id for GHL leads) — only the CALL source
-- needed the GHL union.
