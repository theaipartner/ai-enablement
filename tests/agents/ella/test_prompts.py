...
def test_prompt_advisor_branch_says_dont_escalate():
    ...
    assert "Do NOT emit the [ESCALATE] token" in out
...
def test_prompt_unassigned_advisor_renders_cleanly():
    ...
    client = dict(_CLIENT)
    client["primary_csm"] = {}
    out = build_system_prompt(client, retrieved_chunks=[], speaker=SpeakerIdentity(
        slack_user_id="U_X", display_name="Tester", role="client",
    ))
